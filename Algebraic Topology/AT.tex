...
$$
\begin{tikzcd}[row sep=tiny]
\H_n\br{\S^n} \arrow{r} \arrow[cong]{d} & \H_n\br{\cbr{\text{point}}} \arrow{r} \arrow[cong]{d} & \H_n\br{\S^n} \arrow[cong]{d} \\
...
\hfill
\begin{itemize}
\item $ \S^n = \cbr{\text{point}} \cup \D^n $ attached via the constant map $ \da\D^n \to \cbr{\text{point}} $.
\item $ \S^n = \cbr{\text{point}} \cup \cbr{\text{point}} \cup \D_{\alpha_1}^1 \cup \D_{\alpha_2}^1 \cup \D_{\beta_1}^2 \cup \D_{\beta_2}^2 $.
\item $ \T^2 $ has one $ 0 $-cell, two $ 1 $-cells, and one $ 2 $-cell.
\item $ \Sigma_2 $ has one $ 0 $-cell, four $ 1 $-cells, and one $ 2 $-cell.
\item The \textbf{wedge product}. If $ X $ and $ Y $ are cell complexes, $ X \vee Y = \abr{X \sqcup Y} / x_0 \sim y_0 $ where $ x_0 \in X_0 $ and $ y_0 \in Y_0 $.
\end{itemize}
\end{example*}
...
\item If there exists $ N $ such that $ X = X_N $, $ X $ is a \textbf{finite-dimensional} cell complex.
\item If $ X = X_N $ and $ I_j < \infty $ for all $ j $, $ X $ is a \textbf{finite} cell complex. Then $ X $ is compact.
...
\begin{remark*}
If $ f : \S^n \to \S^n $ is smooth, $ f^{-1}\br{y} $ is finite if $ y $ is a regular value for $ f $, and by Sard's theorem, almost all values, in particular a dense set, are regular.
\end{remark*}
...
\begin{example*}
The real projective space $ \RR\PP^n = \D^n \cup \RR\PP^{n - 1} = \D^n \cup \dots \cup \D^1 \cup \cbr{\text{point}} $ has one cell of each degree $ 0 \le i \le n $, so
$$
\begin{tikzcd}[row sep=tiny]
...
and $ \H_{\cell}^\bullet\br{X} \cong \H_{\cell}\br{X} $ by the exactly analogous argument as for homology.
...
\item If $ A $ and $ B $ are finite cell complexes, $ A \times B $ has a product cell structure such that the open $ i $-cells are of the form $ \br{\text{$ j $-cell in} \ A} \times \br{\text{$ \br{i - j} $-cell in} \ B} $, so $ \chi\br{A \times B} = \chi\br{A}\chi\br{B} $.
\item If $ X = A \cup B $ is a union of two subcomplexes, $ \chi\br{X} = \chi\br{A} + \chi\br{B} - \chi\br{A \cap B} $.
\end{itemize}
\end{example*}
